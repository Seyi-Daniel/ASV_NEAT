...
Important exploration realism:
- We use ACTION REPEAT so that random choices are HELD for multiple env steps.
  That lets the rudder slew and yaw build up (real boats have inertia).
- We start each episode with u0=2 m/s in the environment so the rudder has authority.

This script logs EVERY action at EVERY step for EVERY episode to one CSV:
...
class HParams:
    ...
    action_repeat: int = 10         # << hold same action this many env steps
    save_every: int = 50
    seed: int = 0
    render: bool = False
...
def train(hp: HParams):
    ...
    for ep in range(1, hp.episodes + 1):
        ...
        # --- action repeat state ---
        last_a0 = last_a1 = None
        rep0 = rep1 = 0
        last_rand0 = last_rand1 = False
        last_eps0 = last_eps1 = agent.epsilon()

        for step in range(hp.steps_per_episode):
            if hp.render:
                env.render()

            # sample new action only when repeat counter hits 0
            if rep0 == 0:
                a0, rand0, eps0 = agent.act(obs0)
                last_a0, last_rand0, last_eps0 = a0, rand0, eps0
                rep0 = hp.action_repeat
            else:
                a0, rand0, eps0 = last_a0, last_rand0, last_eps0
            rep0 -= 1

            if rep1 == 0:
                a1, rand1, eps1 = agent.act(obs1)
                last_a1, last_rand1, last_eps1 = a1, rand1, eps1
                rep1 = hp.action_repeat
            else:
                a1, rand1, eps1 = last_a1, last_rand1, last_eps1
            rep1 -= 1

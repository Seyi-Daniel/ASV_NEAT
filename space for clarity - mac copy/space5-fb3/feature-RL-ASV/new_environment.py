#!/usr/bin/env python3
"""
Two-Boat Sectors Environment with Goals + Realistic Small-Boat Dynamics
(1 unit = 1 meter) — rendering scaled with pixels_per_meter

Key realism choices (defendable):
- Surge (forward speed) is governed by thrust minus quadratic drag.
- Yaw (turn rate) follows a first-order Nomoto response with rudder slew limits.
- Yaw authority increases with flow over the rudder (speed or prop wash when accelerating).
- Turning tightens with sustained helm, not instant “heading jumps.”
- Small leeway (sideslip) during turning.
- Actions are helm (left/neutral/right) × throttle (decel/coast/accel) → 9 discrete.

Observations: 12×(closest target per sector, 8 features) + 4 ego = 100 floats.
Episode goal: each boat drives toward a goal point ahead of its initial heading, while
avoiding collision and staying in bounds.

This file **does not** include “turn by N degrees” commitments. That’s not how helms work:
you set a rudder angle; heading changes as a *consequence*. If you want “commitment,”
use action repeat (in training) or a *pilot* that holds rudder for τ seconds—kept out
of the environment on purpose so the RL policy controls it.
"""
...
# =============================
# Ship & Physics  (meters, seconds)
# =============================
@dataclass
class ShipParams:
    # Geometry for a ~20 ft runabout / small RIB
    length: float = 6.0     # m
    width:  float = 2.2     # m
    ...
    min_speed:  float = 0.0
    accel_rate: float = 1.6    # ~0–13.4 m/s in ~8–9 s
    decel_rate: float = 1.0    # reverse thrust (weaker than forward)

    # Yaw/turning (radians, seconds)
    rudder_max:   float = math.radians(25.0)   # ±25°
    rudder_rate:  float = math.radians(40.0)   # slew rate 40°/s
    nomoto_T0:    float = 4.0                  # baseline time constant
    nomoto_T_min: float = 0.6                  # low-speed response floor
    # K is proportional to speed → steady r_ss = K*δ ; with K = K0*U,
    # radius R = U/r_ss ≈ 1/(K0*δ). Choosing K0≈0.115 gives ~20 m radius at δ=25°.
    nomoto_K0:    float = 0.115
    r_max:        float = math.radians(45.0)   # yaw cap

    # Rudder wash (gives some yaw authority when accelerating from rest)
    prop_wash_gain: float = 2.5                # m/s “effective” flow if throttle=accelerate

    # Hydrodynamics
    quad_drag_k:     float = 0.0040   # drag ~ k*u*|u|
...
class Ship:
    def __init__(self, ship_id: int, x: float, y: float, heading: float, speed: float, params: ShipParams):
        ...
        # discrete commands (held between integrates; training can add action_repeat)
        self.last_helm = 0  # -1 right, 0 midships, +1 left
        self.last_thr  = 0  # -1 decel, 0 coast, +1 accel

    @staticmethod
    def decode_action(a: int) -> Tuple[int, int]:
        steer = a // 3                     # 0 none, 1 right, 2 left
        throttle = a % 3                   # 0 coast, 1 accel, 2 decel
    ...
    def integrate(self, dt: float):
        ...
        # 2) Effective flow for yaw dynamics
        U_eff = max(self.u, p.prop_wash_gain * max(thr, 0))

        # 3) First-order Nomoto yaw:  r_dot = (K*delta - r)/T
        ...
        # 5) Kinematics with small leeway (slip) proportional to yaw rate
...
@dataclass
class EnvConfig:
    # Physical world size (meters) — pick any; rendering scales with pixels_per_meter
    world_w: int = 300
    world_h: int = 300

    dt: float = 0.05                # env step (seconds)
    physics_substeps: int = 4       # substeps per env step (stability)
    ...
    show_grid: bool = True
    pixels_per_meter: float = 3.0   # visual only; physics is always meters
...
class TwoBoatSectorsEnv:
    ...
    def reset(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(2):
            while True:
                ...
                u = 2.0  # m/s: small initial speed so rudder has authority
    ...
    def collision_radius(self) -> float:
        # generous, simple circle based on hull length
...
if __name__ == "__main__":
    # Adjust pixels_per_meter/world_w/world_h as you like—physics stays in meters
    ...
    import time
    while True:
        # Try a constant “turn left + accel” to SEE curvature:
        done, info = env.step((7, 7))  # action idx = steer*3+throttle; 7 → left+accel
        env.render()
        if done:
            print("Ended:", info)
            time.sleep(0.8)

...
from .entities import Boat, BoatParams
...
class MultiBoatSectorsEnv:
    ...
    def __init__(self,
        ...
        if cfg.render and HAS_PYGAME:
            self._screen = pygame.display.set_mode((int(self.world_w*self.ppm), int(self.world_h*self.ppm)))
            pygame.display.set_caption("Multi-Boat Sectors v3")
            self._traces = [[] for _ in range(int(self.spawn.n_boats))]

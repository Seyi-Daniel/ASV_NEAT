...
class EntityManager:
    """
    Manages all entities for ALL ASYNC (Vision/Hybrid/Natural/TSS) modes.
    It intelligently selects the correct prompt generator based on the mode.
    """
    ...
    def __init__(self, game_manager, llm_provider='openai'):
        ...
        # --- Use new LLMVisionManager ---
        self.vision_system = LLMVisionManager(provider=llm_provider)
        self.current_scenario = None  # Set in handle_context_selection
        ...
        # Get the mode from the game manager to set the correct log/screenshot paths
        mode = self.game_manager.current_mode
        self.screenshot_dir = f"screenshots_{mode}"
        if not os.path.exists(self.screenshot_dir):
            os.makedirs(self.screenshot_dir)

        self.llm_log_path = f"llm_{mode}_interactions_log.csv"
    ...
    def draw(self, screen):
        ...
        # Hide status box in TSS mode to avoid clutter ---
        # The status box is only drawn if we are NOT in the TSS scenario
        ...
        # title_surface = font.render(f"Vessel Actions ({self.game_manager.current_mode} mode)", True, (0, 0, 0))
    ...
    async def update_vessels(self, dt):
        ...
        # 1. Execute & Update State
        any_move = False
        for v in self.vessels:
            # Check for TSS special case (only applies to Francisco)
        ...
        for v in self.vessels:
            ...
            # Grace Period & TSS Logic
            if conflict_found:
                v.in_maneuver = True
                # Check which vessels should be AI-controlled
                if self.current_scenario == 'tss':
                    # Only Francisco is AI-controlled in TSS
                    if isinstance(v, Francisco):
                        if self._sim_time - self.llm_cooldown.get(id(v), 0.0) >= self.llm_cooldown_duration:
                            to_query.append(v)
                else:
                    # In all other modes, all vessels are AI-controlled
                    if self._sim_time - self.llm_cooldown.get(id(v), 0.0) >= self.llm_cooldown_duration:
                        to_query.append(v)
            else:  # No conflict
        ...
        if to_query:
            ...
            current_mode = self.game_manager.current_mode

            if self.current_scenario == 'tss':
                print(f"===== Generating TSS-specific prompt with history... =====")
            ...
            elif current_mode == 'natural':
                print(f"===== Generating NATURAL LANGUAGE prompt with history... =====")
                text_prompt_for_llm = generate_natural_language_prompt(
                    to_query, list(context_vessels), self.pixels_per_km,
                    previous_vessel_data_list=prompt_history_data,
                    previous_responses=response_history_data
                )
            else:  # Default for 'hybrid' or any other vision mode
                print(f"===== Generating standard (Hybrid) prompt with history... =====")

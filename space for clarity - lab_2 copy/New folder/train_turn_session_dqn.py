#!/usr/bin/env python3
"""
Double-DQN trainer for Multi-Boat Sectors Env (turn-session control, 100-d input)
...
def train(hp: HParams):
    ...
    for ep in range(1, hp.episodes + 1):
        obs_list = env.reset(seed=hp.seed + ep)  # shuffle starts per episode
        ep_ret = 0.0
        reason = ""
        steps  = 0
            ...
            if writer is not None:
                for i in range(hp.n_boats):
                    ...
                    if hp.print_action_log:
                        print(f"EP{ep} STEP{steps} B{i} A={actions[i]} rnd={was_rand[i]} eps={eps_used[i]:.3f} r={rewards[i]:+.3f}")
        ...
        # save light checkpoints (weights + returns array)
